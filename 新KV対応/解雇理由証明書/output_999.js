...
//クリック時のイベント
class ClickAction {
  buttonAction(displayHtml, ledgerNames, displayValue) {
    for (let ledgerName of ledgerNames) {
      this.drawDoubleLine1(ledgerName, displayValue);
  ...
  drawDoubleLine1(ledgerName, displayValue) {
    if (ledgerName == "退職証明書") {
      if (
  ...
  kviewer.events.on("record.show", function (state) {
    ...
    relay
      ...
      .then(() => {
        ...
        action.buttonAction(displayHtml, ledgerNames, displayValue);
      })
      .then(() => {
        let count = 2;
        const maxCount = 30;
        ...
        const intervalId = setInterval(() => {
          let pButton = document.getElementById("解雇理由証明書");
          if (pButton) {
            pButton.click();
